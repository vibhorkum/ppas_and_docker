...
do
    java -jar xdbinstalldir_placeholder/bin/edb-repcli.jar -repsvrfile xdbinstalldir_placeholder/etc/xdb_repsvrfile.conf -addpubdb -dbtype enterprisedb -dbhost ${i} -dbuser enterprisedb -dbpassword `cat xdbinstalldir_placeholder/etc/xdb_repsvrfile.conf | grep pass | cut -f2- -d'='` -database edb -repgrouptype m -dbport  xdbdbport_placeholder -initialsnapshot -replicatepubschema true -changesetlogmode W
done
