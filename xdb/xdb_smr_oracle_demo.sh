...
then
  ...
  docker rm -f xdb_oracle
  docker rm -f xdb_postgres
  exit 0
fi

printf "\e[0;33m==== Building containers for xDB cluster ====\n\e[0m"
  C_NAME="xdb_oracle"
  docker run --privileged=true --publish-all=true --interactive=false --tty=true -v ${PWD}:/xdb_demo -v /Users/${USER}/Desktop:/Desktop --hostname=xdb_oracle --detach=true --name=xdb_oracle wnameless/oracle-xe-11g:latest /bin/bash
