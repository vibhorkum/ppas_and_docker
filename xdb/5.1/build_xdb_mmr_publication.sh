...
java -jar ${XDB_HOME}/bin/edb-repcli.jar -addpubdb -repsvrfile ${XDB_HOME}/etc/xdb_repsvrfile.conf -dbtype enterprisedb -dbhost ${MDN_IP} -dbuser enterprisedb -dbpassword `cat ${XDB_HOME}/etc/xdb_repsvrfile.conf | grep pass | cut -f2- -d'='` -database edb -repgrouptype m -nodepriority 1 -dbport ${XDB_PORT}
...
do
    java -jar ${XDB_HOME}/bin/edb-repcli.jar -repsvrfile ${XDB_HOME}/etc/xdb_repsvrfile.conf -addpubdb -dbtype enterprisedb -dbhost ${i} -dbuser enterprisedb -dbpassword `cat ${XDB_HOME}/etc/xdb_repsvrfile.conf | grep pass | cut -f2- -d'='` -database edb -repgrouptype m -dbport ${XDB_PORT} -initialsnapshot -replicatepubschema true
done
